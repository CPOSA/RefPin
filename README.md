...
调色耗时与图幅成正比，且瓶颈是内存带宽而非算力，因此**实际帧率高度依赖硬件**。
参考实测：Apple Silicon 上 1600×1200 约 6 ms；某 16 核 Windows 笔记本上同尺寸
约 40 ms、3840×2160 约 168 ms（约 6 fps）。后者已在该机内存带宽推算的
理论下限的 1.9 倍以内，属硬件限制而非实现缺陷。详见 [TEST.md](TEST.md) 的 D-001。
